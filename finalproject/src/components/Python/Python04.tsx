...
function Python04() {
  ...
  const [isCompleted, setIsCompleted] = useState<boolean>(
    () => JSON.parse(localStorage.getItem("Python04_isCompleted") || "false")
  );
  const [lastPlayedTime, setLastPlayedTime] = useState<number>(
    () => parseFloat(localStorage.getItem("Python04_lastPlayedTime") || "0")
  );
          ...
          events: {
            ...
            'onStateChange': (event: any) => {
              if (event.data === (window as any).YT.PlayerState.PAUSED || event.data === (window as any).YT.PlayerState.ENDED) {
                ...
                localStorage.setItem("Python04_lastPlayedTime", currentTime.toString());
              }
              if (event.data === (window as any).YT.PlayerState.ENDED) {
                setIsCompleted(true);
                localStorage.setItem("Python04_isCompleted", JSON.stringify(true));
              }
            }
          }
        });
  ...
  return (
    <>
      ...
      <h2>Lecture 4 : Python - Comments</h2>
      {isCompleted && <p>✅ เรียนจบแล้ว</p>} {/* แสดงสถานะการเรียนรู้ */}
    </>
  );
}

export default Python04;
